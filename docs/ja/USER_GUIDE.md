...
### 3.4 Windows タスクスケジューラ / cron 等の定期実行での活用

Windows の「タスクスケジューラ」や Linux の `cron` から定期的にリマインダーや完了通知を起動する場合、**`--timeout` または `--toast`（`-T`）の併用を強く推奨します**。

#### タスクスケジューラ設定時の重要ポイント
1. **セキュリティ オプション**:
   - 必ず **「ユーザーがログオンしているときのみ実行する」** を選択してください。
   - ※「ユーザーがログオンしているかどうかにかかわらず実行する」や `SYSTEM` アカウントを指定すると、Windows の「セッション 0 分離（Session 0 Isolation）」により画面のないバックグラウンドセッションで実行され、デスクトップ上にウィンドウが表示されません。
2. **`--timeout <秒>` の指定（強く推奨）**:
   - 離席中などに通知がポップアップした場合、手動で閉じないとプロセスが常駐し続け、**「次回のタスク実行が重複起動やブロックされてスキップされる」** 原因になります。
   - `--timeout 15` や `--timeout 30` などを設定することで、自動消去され安全に次回実行を迎えられます。

#### タスクスケジューラでの設定例
- **プログラム/スクリプト**: `C:\Tools\MyMsg\MyMsg.exe` （フルパス）
- **引数の追加 (オプション)**: `"定期バックアップが完了しました" -i ok -c green --timeout 20`
- **開始 (オプション)**: `C:\Tools\MyMsg`

#### トースト通知モード（`-T` / `--toast`）の活用
画面中央にウィンドウを出さず、OS の通知センター（画面右下）に記録を残したい場合は、トースト通知モードが最適です：
```powershell

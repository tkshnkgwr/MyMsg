...
## 16. 制約事項およびバックグラウンド・スケジューラ実行仕様

1. **最大待機遅延時間**:
   - `--delay` による待機時間は最大 86400 秒 (24 時間) に制限されます。
2. **同時起動時の挙動**:
   - 本ツールはシングルウィンドウ表示専用であり、複数同時起動時はプロセスごとに独立したウィンドウが生成されます。
3. **Windows セッション 0 分離（Session 0 Isolation）とタスクスケジューラ**:
   - Windows サービスや、タスクスケジューラの「ユーザーがログオンしているかどうかにかかわらず実行する」モード（Session 0）で起動された場合、セキュリティ分離のためデスクトップ画面に GUI ウィンドウは描画されません。
   - タスクスケジューラからポップアップを表示させる場合は、必ず「ユーザーがログオンしているときのみ実行する」でタスクを作成してください。
4. **定期実行時のタイムアウト推奨**:
   - スケジューラや自動バッチで起動する場合は、プロセスの残留および次回タスクの重複実行ブロックを防ぐため、`--timeout <秒>` または `--toast`（`-T`）を指定することが推奨されます。
